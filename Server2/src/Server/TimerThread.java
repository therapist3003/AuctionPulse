...
public class TimerThread implements Runnable{
	...
	public void run() {
		...
		cnt=1;
		synchronized(ServerCollection2.players.keySet()) {
		for(Integer n:ServerCollection2.players.keySet())
		{
			ID=n;
			player =ServerCollection2.players.get(n);
			try 
			{
				Thread.sleep(61000);
			...
			try 
			{
				...
				if(bid.Bid_Price!=-1)
				{
					Jdbc.InsertBidder(ID,bid.Bidder_Name,bid.Bid_Price,bid.Bidder_ID);
					array[cnt]=n;
					cnt++;
				}
			} catch (Exception e) 
		...
		for(int i=0;i<cnt;i++)
		{
			ServerCollection2.players.remove(array[i]);
		}
		current.Time="null";
    	current.ID=0;
    	try 
    	{
    		Jdbc.AuctionStopped();
    		Jdbc.InsertCurrentPlayer(current);
