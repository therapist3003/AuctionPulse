...
public class Purchase extends JPanel{
	...
	public String player, player2, player3;
	public Purchase thisobject;
	public JLabel lblFillTheFollowing,lblName,lblcost,lblphoto,lblThisChoiceIs2,label_1,label_2,label_3,label,lblYourCost, lblPlayerCost,lblname,lblNewLabel,lblname2;
	...
	public Player sentplayer;
	public FileOutputStream fos;
	public ImageIcon image;
	public byte Barray[]=new byte[120000];   //120 KBytes
	
	public Purchase(ObjectInputStream input, ObjectOutputStream output, BidderInfo bidder, NotStarted notStarted) throws Exception {
		...
		lblFillTheFollowing = new JLabel("Enter your cost for this Player");
		...
		lblPlayerCost = new JLabel("Reserved Cost");
		lblPlayerCost.setFont(new Font("Serif", Font.PLAIN, 20));
		lblPlayerCost.setBounds(406, 149, 153, 25);
		add(lblPlayerCost);
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e2) {
				...
				sendcost.Bidder_Name=bidder.name;
				sendcost.cost=cost;
				sendcost.ID=ID;
				sendcost.Consumer_ID=bidder.ID;
				try 
				{
					synchronized(output)
		    		{
		...
		lblPlayerId.setBounds(124, 295, 70, 25);
		lblPlayerId.setFont(new Font("Serif", Font.BOLD, 15));
		add(lblPlayerId);
	...
	public void refresh() throws Exception
	{
		...
		if(status && !flag)
		{
			...
			BidderCollection.PlayerList = PlayerList;
			...
			while(it.hasNext())
			{
				sentplayer =it.next();
				Barray= sentplayer.b;
				File file=new File("/home/tharun30/Package_sem5/Online-Auction-System-master/Bidder/BidderImages/"+String.valueOf(sentplayer.ID)+".txt");
				fos=new FileOutputStream(file);
				fos.write(Barray);
				image=new ImageIcon("/home/tharun30/Package_sem5/Online-Auction-System-master/Bidder/BidderImages/"+String.valueOf(sentplayer.ID)+".txt");
				BidderCollection.players.put(sentplayer.ID, sentplayer);
				BidderCollection.Images.put(sentplayer.ID,image);
			}
